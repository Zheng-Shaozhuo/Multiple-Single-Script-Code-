...
class DbClient(object):
    ...
    def getInstance(cls, *args, **kwargs):
        ...
        if _key not in DbClient.instances:
            DbClient.instances[_key] = DbClient(*args, **kwargs)
    ...
    def db_connect(self):
        ...
        try:
            ...
            if self.rettype == 'dict':
                self.cursor_type = MySQLdb.cursors.DictCursor
            else:
                self.cursor_type = MySQLdb.cursors.Cursor
            self.cursor = self.conn.cursor(cursorclass=self.cursor_type)
    ...
    def insert(self, table, params):
        ...
        properties = ','.join(MySQLdb.escape_string(s) for s in params.keys())
    ...
    def select_count(self, table, conds='1=1'):
        ...
        if self.cursor_type == MySQLdb.cursors.Cursor:
            count = int(ret[0][0])
        else:
    ...
    def validateConds(self, conds):
        ...
        if isinstance(conds, dict):
            ...
            for _k, _v in conds.items():
                cond_list.append("{}='{}'".format(MySQLdb.escape_string(str(_k)), MySQLdb.escape_string(str(_v))))
